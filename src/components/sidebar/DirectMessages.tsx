      ...
      {isExpanded && (
        <ul className="space-y-1">
          {Array.from(activeDMs).map((userId) => {
            ...
            const dmChannel = `dm-${[currentUser, userId].sort().join("-")}`;

            return (
              <li key={userId}>
                <button
                  className={`w-full text-left flex items-center space-x-2 hover:bg-chat-hover rounded p-2 ${
                    activeChannel === dmChannel ? "bg-chat-hover" : ""
                  }`}
                  onClick={() => {
